...
export function SourcePatchCard({ patch, onConfirm, onCancel }: SourcePatchCardProps) {
  ...
  const confirm = () => {
    ...
    if (!result.ok)
      setError(
        result.error === 'fragment-missing' ||
          result.error === 'raw-changed' ||
          result.error === 'revision-changed'
  ...
  return (
    <section className="source-patch-card" aria-label={t('sourcePatchTitle')}>
      ...
      <div className="source-patch-diff" role="table">
        {Array.from({ length: count }, (_, index) => (
          <div className="source-patch-line" role="row" key={index}>
            <code className="source-patch-before" role="cell">
              {before[index] ?? ''}
            </code>
            <code className="source-patch-after" role="cell">
              {after[index] ?? ''}
            </code>
          </div>
        ))}
      ...
      {error && (
        <p className="source-patch-error" role="alert">
          {error}
        </p>
      )}
