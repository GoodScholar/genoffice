...
function normalizeMathHtml(html: string): string {
  ...
  for (const annotation of root.querySelectorAll('annotation[encoding="application/x-tex"]')) {
    ...
    carrier.replaceWith(
      mathElement(latex, carrier.matches('.katex-display, math[display="block"]')),
    )
  }
      ...
      new Plugin({
        props: {
          clipboardTextParser: (text, context, plain) => {
            if (!plain && /[$\\]/.test(text) && editor.markdown) {
              const parsed = editor.schema.nodeFromJSON(
                editor.markdown.parse(normalizePastedMath(text)),
              )
              let hasMath = false
              ...
              if (hasMath) return Slice.maxOpen(parsed.content)
            }
            // Match ProseMirror's default text parser, including active marks.
            const root = document.createElement('div')
